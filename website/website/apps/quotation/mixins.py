...
class QuotationPlacementMixin(CheckoutSessionMixin):
    """
    Mixin which provides functionality for placing quotes.
    ...
    def generate_quotation_number(self, basket):
        """
        Return a new quote number
        """
        return QuotationNumberGenerator().quotation_number(basket)

    def handle_quotation_placement(self, quotation_number, user, basket,
                                   shipping_address, shipping_method,
                                   shipping_charge, billing_address, order_total,
                                   **kwargs):
        """
        Write out the quote models and return the appropriate HTTP response

        We deliberately pass the basket in here as the one tied to the request
        isn't necessarily the correct one to use in requesting the quote.  This
        can happen when a basket gets frozen.
        """
        quotation = self.place_quotation(
            quotation_number=quotation_number, user=user, basket=basket,
        ...
        basket.freeze()
        return self.handle_successful_quote(quotation)

    def place_quotation(self, quotation_number, user, basket, shipping_address,
        ...
        quotation = QuotationCreator().place_quotation(
            user=user,
        ...
        # self.save_payment_details(quotation)
        return quotation
    ...
    def handle_successful_quote(self, quotation):
        """
        ...
        self.send_confirmation_message(quotation, self.communication_type_code)

        # Flush all session data
        self.checkout_session.flush()

        # Save order id in session so thank-you page can load it
        self.request.session['checkout_quotation_id'] = quotation.id

        response = HttpResponseRedirect(self.get_success_url())
        self.send_signal(self.request, response, quotation)
        return response

    def send_signal(self, request, response, quotation):
        self.view_signal.send(
            sender=self, quotation=quotation, user=request.user,
            request=request, response=response)

    def get_success_url(self):
        return reverse('quotation:request-quote')

    def send_confirmation_message(self, quotation, code, **kwargs):
        ctx = self.get_message_context(quotation)
        try:
    ...
    def get_message_context(self, quotation):
        ctx = {
        ...
        if not self.request.user.is_authenticated():
            ...
            try:
                path = reverse('customer:anon-order',
                               kwargs={'order_number': quotation.number,
                                       'hash': quotation.verification_hash()})
