...
class CustomerApplication(Application):
    ...
    def get_urls(self):
        urls = [
            ...
            # Quote requests
            url(r'quotation/$', login_required(self.quotation_history_view.as_view()), name='quotation-list'),
            # url(r'^quote-status/(?P<order_number>[\w-]*)/(?P<hash>\w+)/$',
            #     self.anon_order_detail_view.as_view(), name='anon-order'),
            url(r'^quotation/(?P<number>[\w-]*)/$',
                login_required(self.quotation_detail_view.as_view()),
                name='quotation'),
            url(r'^quotation/(?P<number>[\w-]*)/(?P<line_id>\d+)$',
                login_required(self.order_line_view.as_view()),
                name='quotation-line'),
...
# from oscar.core.loading import get_class
#
# from .views import QuotationHistoryView
#
#
# class CustomerApplication(CoreCustomerApplication):
#     name = 'customer'
#     # quotation_history_view = get_class('customer.views', 'QuotationHistoryView')
#     quotation_history_view = QuotationHistoryView
#
#     def get_urls(self):
#         urls = [
#             # url(include(CoreCustomerApplication.urls)),
#             url(r'quotations/$', login_required(self.quotation_history_view.as_view()), name='quotation-list'),
#         ]
#         return urls
#
# application = CustomerApplication()
